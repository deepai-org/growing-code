...
#
# Three changes over grow_v6 (lexicase selection):
#
#   1. Annealed Macro-Mutations: High rate early (15%) for structural
#      discovery (loop wraps, mod-if blocks), decaying to near-zero
#      for register wiring. Separates exploration from exploitation.
#
#   2. MAP-Elites Structural Archive: Programs archived by structural
#      traits (loop count x mod count x register diversity). Protects
#      structurally interesting but poorly-wired programs from being
#      outcompeted by simple optimizers. Archive members injected
#      into the population periodically.
#
#   3. Fuzzy Jump Matching: up/down N matches mark N +/- 1, taking
#      the nearest match. Inspired by Tierra/Avida template matching.
#      Makes jump wiring resilient to operand mutations — a single
#      mutation shifts a jump target instead of breaking it entirely.
#
# Usage: perl grow_v8.pl [target] [generations] [seed]
# Example: perl grow_v8.pl primes 3000
#          perl grow_v8.pl fib 2000 42
...
print "=" x 60 . "\n";
print "grow_v8.pl — Annealed Macros + MAP-Elites + Fuzzy Jumps\n";
print "=" x 60 . "\n";
...
sub run_program {
	...
	while ($pc >= 0 && $pc < @ins && $steps++ < $MAX_STEPS) {
		...
		if    ($op eq 'mark') { $acc = $arg }
			...
			# Fuzzy jump matching: match mark N where |N - arg| <= 1
			# Prefers exact match (distance 0), falls back to ±1.
			# Makes jump wiring resilient to single operand mutations.
			my $dir = ($op eq 'up') ? -1 : 1;
			my $scan = $pc;
			my $found = 0;
			my $best_scan = -1;
			...
			while (1) {
				...
				if ($ins[$scan][0] eq 'mark') {
					my $d = abs($ins[$scan][1] - $arg);
					if ($d < $best_dist) {
						$best_dist = $d;
						$best_scan = $scan;
						last if $d == 0;  # exact match, stop immediately
					}
				}
			}
			if ($best_scan >= 0 && $best_dist <= 1) {
				$pc = $best_scan;
				$found = 1;
			}
			next if $found;
...
# === MACRO-MUTATIONS (structural building blocks) ===
# These inject pre-formed structural patterns that point mutations
# can't create incrementally. Called with annealing: high rate early
# (discovery), decaying to near-zero late (wiring/exploitation).
sub macro_mutate {
	my $genome = shift;
	my @g = map { [$_->[0], $_->[1]] } @$genome;

	if (rand() < 0.5) {
		# LOOP WRAP: wrap a random block in mark N ... up N
		...
		# MOD-IF INSERT: inject [mark A / setv B / mod C / if A]
...
# === STRUCTURAL FEATURES (for MAP-Elites archive) ===
# Classifies a genome by its structural traits, not output.
sub structural_features {
	my ($genome) = @_;
	my @g = @$genome;

	# Count mod instructions
	my $mod_count = 0;
	for (@g) { $mod_count++ if $_->[0] eq 'mod' }

	# Count distinct registers referenced
	my %regs;
	for (@g) { $regs{$_->[1]}++ }
	my $reg_count = scalar keys %regs;

	# Count loop structures: mark N with a later up N (or ±1 for fuzzy)
	my %mark_pos;
	for my $i (0..$#g) {
		$mark_pos{$g[$i][1]} //= $i if $g[$i][0] eq 'mark';
	}
	my $loop_count = 0;
	for my $i (0..$#g) {
		if ($g[$i][0] eq 'up') {
			for my $target ($g[$i][1]-1 .. $g[$i][1]+1) {
				if (exists $mark_pos{$target} && $mark_pos{$target} < $i) {
					$loop_count++;
					last;
				}
			}
	...
	# Discretize into MAP-Elites bin key
	my $loop_bin = min(3, $loop_count);              # 0, 1, 2, 3+
	my $mod_bin  = min(3, int($mod_count / 1.5));    # 0, 1, 2, 3+
	my $reg_bin  = min(2, int(($reg_count - 1) / 3)); # 0-3, 4-6, 7+

	return {
		loop_count => $loop_count,
		mod_count  => $mod_count,
		reg_count  => $reg_count,
		bin_key    => "$loop_bin,$mod_bin,$reg_bin",
	};
}

# === MAP-ELITES ARCHIVE ===
my %ARCHIVE;  # bin_key => { genome, fitness, features }

sub archive_try_insert {
	my ($genome, $fitness, $out) = @_;
	my $feat = structural_features($genome);
	my $key = $feat->{bin_key};

	if (!$ARCHIVE{$key} || $fitness > $ARCHIVE{$key}{fitness}) {
		$ARCHIVE{$key} = {
			genome   => [ map { [$_->[0],$_->[1]] } @$genome ],
			fitness  => $fitness,
			features => $feat,
			out      => $out,
		};
...
sub archive_sample {
	my @entries = values %ARCHIVE;
	return undef unless @entries;
	# Prefer structurally interesting cells (high loop + mod count)
	my @weighted;
	for my $e (@entries) {
		my $weight = 1 + $e->{features}{loop_count} * 3 + $e->{features}{mod_count} * 2;
		push @weighted, ($e) x $weight;
	}
	return $weighted[int(rand(@weighted))];
}
...
sub lexicase_generation {
	my ($pop_ref, $target_ref, $soup, $tribe_results, $gen, $state, $phase) = @_;
	my @pop = @$pop_ref;
	my @tgt = @$target_ref;
	$phase //= 0.5;
	...
	for my $genome (@pop) {
		...
		# Insert into MAP-Elites archive
		archive_try_insert($genome, $fit, $out);
	}
	...
	# Extinction event — restock with archive members + tribe graduates
	if ($state->{stagnation} > $EXTINCT_THRESH) {
		...
		while (@new_pop < $POP_SIZE) {
			my $r = rand();
			if ($r < 0.20) {
				# Pull from MAP-Elites archive (structurally interesting)
				my $entry = archive_sample();
				if ($entry) {
					push @new_pop, [ map { [$_->[0],$_->[1]] } @{$entry->{genome}} ];
					next;
				}
			}
			if ($tribe_results && $r < 0.40) {
		...
		printf "Gen %d: !! EXTINCTION !! (archive:%d cells)\n", $gen, scalar keys %ARCHIVE;
		return \@new_pop;
	}
	...
	# Inject archive members (structurally diverse programs)
	if ($gen % 10 == 0 && keys %ARCHIVE > 0) {
		for (1..3) {
			my $entry = archive_sample();
			next unless $entry;
			my $idx = $ELITE_SIZE + int(rand(@scored - $ELITE_SIZE));
			next if $idx >= @scored;
			$scored[$idx]{genome} = [ map { [$_->[0],$_->[1]] } @{$entry->{genome}} ];
		}
	}
	...
	# LEXICASE SELECTION + ANNEALED MACRO-MUTATIONS
	# Macro rate: 15% early (phase=0) → 0.5% late (phase=1)
	my $macro_rate = max(0.005, 0.15 * (1.0 - $phase));
	...
	while (@next < $POP_SIZE) {
		...
		if (rand() < $CROSSOVER_RATE) {
			...
			$child = homologous_cross($p1->{genome}, $p2->{genome});
		} else {
			my $parent = lexicase_select(\@scored, \@tgt);
			$child = [ map { [$_->[0],$_->[1]] } @{$parent->{genome}} ];
		}

		# Apply mutation: macro (annealed) or point
		if (rand() < $macro_rate) {
			push @next, mutate(macro_mutate($child));
		} else {
			push @next, mutate($child);
		}
	}
...
if ($targ_name eq 'primes') {
	...
	printf "Macros: 15%% → 0.5%% (annealed)  Archive: MAP-Elites  Jumps: fuzzy +-1\n\n";
	...
	for my $g (1..$merger_gens) {
		...
		my $phase = $g / $merger_gens;  # 0.0 → 1.0 (explore → exploit)
		$pop_ref2 = lexicase_generation($pop_ref2, \@TARGET, $soup, \%tribe_results, $gen, $state, $phase);
	}
	...
	for my $stage_idx (0..$#stages) {
		...
		if (!$is_final) {
			...
			for my $g (1..$target_gens) {
				my $gen = $gen_offset + $g;
				my $phase = $g / $target_gens;  # 0.0 → 1.0
				$pop_ref = lexicase_generation($pop_ref, \@TARGET, $soup, undef, $gen, $state, $phase);
			}
