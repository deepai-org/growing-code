...
#
# Builds on grow_v5 (tribal evolution, soup, crossover) and replaces
# the selection mechanism with Lexicase Selection.
#
# Instead of aggregating fitness into one number (which kills specialists),
# lexicase selects parents by shuffling test cases and filtering:
#   1. Shuffle test case order randomly
#   2. Keep only programs with best performance on case 1
#   3. From survivors, keep only best on case 2
#   4. Repeat until one remains (or pick randomly from survivors)
#
# This naturally protects "specialists" — a program that correctly
# rejects 25 as composite survives even if its overall output is messy,
# because on THAT specific test case it outperforms the population.
#
# Replaces fitness sharing (lexicase inherently promotes diversity).
#
# Usage: perl grow_v6.pl [target] [generations] [seed]
# Example: perl grow_v6.pl primes 3000
#          perl grow_v6.pl fib 2000 42
...
print "=" x 60 . "\n";
print "grow_v6.pl — Lexicase Selection\n";
print "=" x 60 . "\n";
